...
function filterVIP(members) {
  ...
  const VIPMembers = members.filter((item, index, array) => item.level === "VIP");
  return VIPMembers;
}
...
function sumCredits(members) {
  ...
  const sum = members.reduce((acc, item, index, array) => {
...
async function getVIPSummary(filePath) {
  ...
  const totalMembers = await readMembers(filePath);
  const membersAsVIP = filterVIP(totalMembers);
  const totalCredits = sumCredits(membersAsVIP);
  const totalNames = membersAsVIP.map((item, index, array) => item.name);
  return {
    count: membersAsVIP.length,
    totalCredits,
    names: totalNames
  }
}
